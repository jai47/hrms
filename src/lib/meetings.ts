...
export type UpcomingMeeting = Awaited<ReturnType<typeof getUpcomingMeetings>>[number]
...
export async function getUpcomingMeetings(employeeId?: string, limit = 5) {
  if (employeeId) {
    return getUpcomingMeetingsForEmployee(employeeId, limit)
  }
  return getUpcomingMeetingsAll(limit)
}
